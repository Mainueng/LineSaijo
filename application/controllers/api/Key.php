...
//    ];
    ...
    public function index_put()
    {
        ...
        if ($this->_insert_key($key, array('level'=>$level,'ignore_limits'=>$ignore_limits))) {
            $this->response(array('status'=>true,'key'=>$key),REST_Controller::HTTP_CREATED);
        } else {
            $this->response(array('status'=>false,'message'=>'Could not save the key'),REST_Controller::HTTP_INTERNAL_SERVER_ERROR);
        }
    }
    ...
    public function index_delete()
    {
        ...
        if (!$this->_key_exists($key)) {
            ...
            $this->response(array('status'=>false,'message'=>'Invalid API key'),REST_Controller::HTTP_BAD_REQUEST);
        ...
        // Respond that the key was destroyed
          ...
          $this->response(array('status'=>true,'message'=>'API key was deleted'),REST_Controller::HTTP_NO_CONTENT);
    ...
    public function level_post()
    {
        ...
        if ($this->_update_key($key,array('level'=>$new_level))) {
            ...
            $this->response(array('status'=>true,'message'=>'API key was updated'),REST_Controller::HTTP_OK);

        } else {

            $this->response(array('status'=>false,'message'=>'Could not update the key level'),REST_Controller::HTTP_INTERNAL_SERVER_ERROR);
    ...
    public function suspend_post()
    {
        ...
        if ($this->_update_key($key,array('level'=>0))) {
            $this->response(array('status'=>true,'message'=>'Key was suspended'),REST_Controller::HTTP_OK);

        } else {
            $this->response(array('status'=>false,'message'=>'Could not suspend the user'),REST_Controller::HTTP_INTERNAL_SERVER_ERROR);
