...
class Product_model extends MY_Model
{
    ...
    public function device_list()
    {
        ...
        $query = $this->db3->query("select cus_air_mn.*, " .
           "air_info.serial as serial,air_info.energy as air_energy,air_info.indoor as indoor,air_info.outdoor as outdoor,air_info.datetime as air_datetime,air_info.iot, ".
           "TA.* ".
           "from cus_air_mn ".  
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($query->result_array() as $row) {
                $rows = array(
                    'air_serial' => get_array_value($row, 'serial', ''),
                    'air_name' => trim(subairname(get_array_value($row, 'indoor', ''))),
                    'air_ind_conf' => sub_indoor_conf(get_array_value($row, 'air_energy', '')),
                    'air_ip' => trim(subip(get_array_value($row, 'air_energy', ''))),
                    'air_ssid' => trim(subssid(get_array_value($row, 'air_energy', ''))),
                    'air_wifi' => substr(bin2hex(get_array_value($row, 'air_energy', '')),30,2),
                    'air_mgr_latitude' => get_array_value($row, 'latitude', ''),
                    'air_mgr_longitude' => get_array_value($row, 'longitude', ''),
                    'air_mgr_group' => get_array_value($row, 'group', ''),
                    'air_mgr_enable' => get_array_value($row, 'enable', ''),
                    ...
                    'air_datetime' => get_array_value($row, 'air_datetime', ''),
                );
                ...
                $this->db3->where('serial', $rows['air_serial']);
                ...
                if ($msg == 1) {
                    ...
                    $duration = 30;
                    $dateinsec = strtotime($rows['air_datetime']) + $duration;
                    ...
                    $airinfo = array(
                        'ip' => $rows['air_ip'],
                        'name' => $rows['air_name'],
                        'serial' => $rows['air_serial'],
                        "ssid" => $rows['air_ssid'],
                        "status" => $online_status,
                        "latitude" => $rows['air_mgr_latitude'],
                        "longitude" => $rows['air_mgr_longitude'],
                        "group" => $rows['air_mgr_group'],
                        "enable" => $rows['air_mgr_enable'],
                        "ind_conf" => $rows['air_ind_conf'],
                        "ver" => $rows['air_wifi'],
                    ...
                    $this->db->like('serial',$rows['air_serial'],'both');
    ...
    public function check_device()
    {
        ...
        if ($query->num_rows() > 0) {
            ...
            $data['ssid'] = trim(subssid($result->energy));
            $data['ip'] = trim(subip($result->energy));
            $data['time_on'] = subtimeon($result->indoor);
            $data['time_off'] = subtimeoff($result->indoor);
            $data['name'] = trim(subairname($result->indoor));
            $data['password'] = trim(subairpass($result->indoor));
            $data['air_type_id'] = subairtype($result->indoor);
            //$data['model'] = subairmodel($result->indoor);

            $this->db3->where('serial', $this->_serial);
            $this->db3->update($this->tbl_air_info, $data);
    ...
    public function update_cmd()
    {
        ...
        if ($query->num_rows() > 0) {
            ...
            if ($cmd_req == '43') {
                ...
                $sql = "INSERT INTO cus_air_cmd (cus_mstr_id, air_info_serial,cmd, datetime) VALUES (".$this->_id.", '".$this->_serial."', 0x".$this->_command.", '".$today."')";

                $this->db3->query($sql);

                /*Update command info*/
                $sql = "UPDATE cus_air_mn SET cmd = 0x".$this->_command.", datetime = '".$today."' WHERE cus_mstr_id = ".$this->_id." AND air_info_serial = '".$this->_serial."'";
                $this->db3->query($sql);
