<div id="container">
	...
	<div id="content">
		<div class="page-header">
			<div class="container-fluid">
				<h1 class="pl-3">Technicain Infomation</h1>
				<div class="float-right ml-1">
					<a href="<?php echo site_url('technician-list'); ?>" data-toggle="tooltip" data-placement="top" title="Cancel" class="btn btn btn-default font-14"><i class="fas fa-reply"></i></a>
				</div>
			...
			<div class="col-12">
				<div class="card">
					<div class="card-header">
						<i class="fas fa-user-edit"></i> Technicain Infomation Form
					</div>
					<div class="card-body">
						<form class="form-inline" action="<?php echo site_url('technician-list/form/'.$this->data['user_id']); ?>" method="post" enctype="multipart/form-data">
							...
							<div class="form-group w-100 required">
								...
								<div class="col-10 form-input">
									...
									<small class="error-text pt-2 d-none">Telephone must be between 9 or 10 characters!</small>
								</div>
							</div>
							<div class="form-group w-100 required">
								...
								<div class="col-10 form-input upload-btn-wrapper">
									...
									<img id="profile-img" src="<?php echo base_url().'application/controllers/v1/club/upload/profile_img/'.$this->data['profile_img']; ?>">
								</div>
							</div>
							<div class="form-group w-100 required">
							...
							<div class="form-group w-100 required">
								...
								<div class="col-10 form-input">
									<select class="form-control font-14 w-100" id="dealer_store" name="dealer_store">
										<option value=""></option>
										<?php 

										foreach ($this->data['dealer_list'] as $result) {
											if ($result['dealer_store'] == $this->data['dealer_store']) {
												echo '<option value="'.$result['dealer_store'].'" selected>'.$result['dealer_store'].'</option>';
											} else {
									...
									<small class="error-text pt-2 d-none">Dealer Store must be between 1 and 32 characters!</small>
								</div>
							</div>
							<div class="form-group w-100 required">
								<label class="col-2 control-label" for="input-rating">Rating</label>
								<div class="col-10 form-input">
									<input type="text" name="rating" value="<?php echo $this->data['rating']; ?>" id="input-rating" class="form-control w-100" placeholder="Rating" readonly="">
								</div>
							</div>
							<div class="form-group w-100 required">
								<label class="col-2 control-label" for="input-saijo_certification">Saijo Certification</label>
								<div class="col-10 form-input">
									<input type="text" name="saijo_certification" value="<?php echo $this->data['saijo_certification']; ?>" id="input-saijo_certification" class="form-control w-100" placeholder="Saijo Certification" readonly>
								</div>
							</div>
							<div class="form-group w-100 required">
...
<script type="text/javascript">
	$(".btn-primary").click(function(){
		...
		if ($('#input-address').val() && $('#input-address').val().length <= 200) {
		...
		if ($('#input-district').val() && $('#input-district').val().length <= 32) {
		...
		if ($('#input-province').val() && $('#input-province').val().length <= 32) {
		...
		if ($('#input-password').val() != '') {
			...
			if (($('#input-password').val() && $('#input-confirm_password').val()) && ($('#input-password').val() == $('#input-confirm_password').val())) {
				...
				$('html,body').animate({
					scrollTop: $("#input-password").offset().top
				}, 1500);
			}
		}

		if (($('#input-name').val() && $('#input-name').val().length < 32 &&
			$('#input-lastname').val() && $('#input-lastname').val().length < 32 &&
			$('#input-telephone').val() && $('#input-telephone').val().length > 8 && $('#input-telephone').val().length <= 10 &&
			$('#input-address').val() && $('#input-address').val().length <= 200 &&
			$('#input-district').val() && $('#input-district').val().length <= 32 &&
			$('#input-province').val() && $('#input-province').val().length <= 32 &&
			$('#input-postal_code').val() && $('#input-postal_code').val().length == 5 &&
			$('#input-user_name').val()) && ($('#input-password').val() == $('#input-confirm_password').val())) {
			$("form").submit();
	}
